...
interface MuralProps {
  ...
  initialPostData?: (Partial<MuralPost> & { autoCreateTreatment?: boolean }) | null;
  onClearInitialPostData?: () => void;
  initialFilters?: { search?: string, mentions?: boolean, criticality?: string };
}
...
const Mural: React.FC<MuralProps> = ({ 
  ...
  const [filterCriticality, setFilterCriticality] = useState<string>(initialFilters?.criticality || 'ALL');
  const [filterStatus, setFilterStatus] = useState<string>('ALL');
  const [filterSearch, setFilterSearch] = useState(initialFilters?.search || searchTerm);
  const [filterPeriod, setFilterPeriod] = useState<number>(7);
  const [showMyMentions, setShowMyMentions] = useState(initialFilters?.mentions || false);

  // Update filters when initialFilters changes
  useEffect(() => {
    if (initialFilters) {
      if (initialFilters.criticality) setFilterCriticality(initialFilters.criticality);
      if (initialFilters.search) setFilterSearch(initialFilters.search);
      if (initialFilters.mentions !== undefined) setShowMyMentions(initialFilters.mentions);
    }
  }, [initialFilters]);
  ...
  useEffect(() => {
    if (initialPostData) {
      setNewPost(prev => ({
        ...prev,
        ...initialPostData,
        title: initialPostData.title || prev.title,
        description: initialPostData.description || prev.description,
        caseId: initialPostData.caseId || prev.caseId,
        subject: initialPostData.subject || prev.subject,
        criticality: initialPostData.criticality || prev.criticality,
        type: initialPostData.type || (initialPostData.criticality as any) || prev.type,
      }));
      if (initialPostData.autoCreateTreatment) {
        setAutoCreateTreatment(true);
      }
      setIsNewPostModalOpen(true);
      onClearInitialPostData?.();
    }
  }, [initialPostData, onClearInitialPostData]);
  ...
  const filteredPosts = useMemo(() => {
    ...
    return posts.filter(post => {
      ...
      if (!showArchived && post.isArchived) return false;
      if (showArchived && !post.isArchived) return false;

      // Automatic Archiving (Legacy logic, maybe keep as fallback or remove)
      const postDate = new Date(post.createdAt);
      const daysOld = differenceInDays(today, postDate);
      if (!post.isArchived && daysOld > 15 && !post.isPinned && post.criticality !== 'Crítico' && post.status === 'Encerrado') return false;
  ...
  const handleAddPost = () => {
    ...
    if (autoCreateTreatment) {
      const newTreatment: MuralTreatment = {
        ...
        priority: newPost.criticality === 'Crítico' ? 'Crítico' : 'Ação necessária',
    ...
    if (needsTreatment && !autoCreateTreatment) {
        ...
        <div className="flex flex-col gap-1 flex-1 w-full max-w-xl">
          ...
          <div className="flex items-center gap-4 mt-2 px-1">
            ...
            <button 
              ...
              className={`text-[10px] font-black uppercase transition-colors flex items-center gap-1.5 ${showArchived ? 'text-amber-600' : 'text-slate-400 hover:text-slate-600'}`}
            >
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4 animate-in fade-in duration-300">
          <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-xl overflow-hidden border-[8px] border-white transform animate-in zoom-in-95 duration-300">
            ...
            <div className="p-8 space-y-6">
              ...
              {autoCreateTreatment && (
                <div className="bg-indigo-50 border-2 border-indigo-100 rounded-2xl p-4 flex items-center justify-between animate-pulse">
                  <div className="flex items-center gap-3">
                    <CheckCircle2 className="w-5 h-5 text-indigo-600" />
                    <div>
                      <p className="text-[10px] font-black text-indigo-900 uppercase">Modo Automação Ativo</p>
                      <p className="text-[9px] font-bold text-indigo-700">Uma tratativa será criada e vinculada automaticamente.</p>
                    </div>
                  </div>
                  <button 
                    onClick={() => setAutoCreateTreatment(false)}
    ...
    <div className={`bg-white rounded-2xl border transition-all ${
          ...
          <div className="relative" ref={menuRef}>
            ...
            {isMenuOpen && (
              <div className={`absolute right-0 ${menuPosition === 'down' ? 'top-full mt-1' : 'bottom-full mb-1'} bg-white border border-slate-200 rounded-xl shadow-xl z-[150] w-48 overflow-hidden animate-in fade-in slide-in-from-${menuPosition === 'down' ? 'top' : 'bottom'}-2`}>
                ...
                >
                  <Pin className={`w-3 h-3 ${post.isPinned ? 'text-blue-500 fill-current' : ''}`} />
                ...
                <button 
                  ...
                  className="w-full px-4 py-2 text-left text-[10px] font-black uppercase text-[#003DA5] hover:bg-blue-50 flex items-center gap-2"
                >
                  <Plus className="w-3.5 h-3.5" />
                  {treatment ? 'Ver/Editar Tratativa' : 'Abrir Tratativa'}
                </button>

                <button 
                  onClick={() => {
                    setIsMenuOpen(false);
                    onOpenCase(post.subject || post.caseId || post.title);
                  }}
                  className="w-full px-4 py-2 text-left text-[10px] font-black uppercase text-slate-600 hover:bg-slate-50 flex items-center gap-2"
                >
                ...
                >
                  ...
                  {isCommenting ? 'Ocultar Comentários' : `Comentários (${post.comments.length})`}
                </button>
                ...
                <button 
                  onClick={() => {
                    onUpdate({ 
                      ...
                      status: !post.isArchived ? 'Encerrado' : 'Aberto' 
                    });
                    setIsMenuOpen(false);
                  }}
                  className="w-full px-4 py-2 text-left text-[10px] font-black uppercase text-slate-600 hover:bg-slate-50 flex items-center gap-2"
                ...
                <button 
                  onClick={() => {
                    if (confirm('Deseja realmente remover esta postagem do mural?')) {
                      onUpdate({ 
                        ...post, 
                        isRemoved: true, 
      ...
      <div className="p-4">
        <div className="flex flex-col gap-1">
          <h3 className="text-sm font-black text-slate-800 leading-tight uppercase tracking-tight line-clamp-1">{post.title}</h3>
          
          {treatment && (
            <div className="flex items-center gap-1.5 text-[9px] font-black text-blue-600 uppercase mt-1">
              <ClipboardList className="w-3 h-3" />
              Monitoramento Ativo: {treatment.responsible}
            </div>
          )}
        </div>
        ...
        <div className="mt-4 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <button 
              onClick={() => setIsAgirMenuOpen(true)}
              className="px-6 py-2 bg-emerald-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-emerald-700 transition-all flex items-center gap-2 shadow-lg shadow-emerald-900/10 active:scale-95 group/agir"
            >
              <ShieldCheck className="w-4 h-4 group-active/agir:scale-110 transition-transform" />
              Agir
            </button>

            <button 
              onClick={() => setIsTreatmentOpen(true)}
              className="px-4 py-2 bg-blue-50 text-[#003DA5] rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-blue-100 transition-all flex items-center gap-2 active:scale-95"
            >
              <Plus className="w-3.5 h-3.5" />
              {treatment ? 'Ver Tratativa' : 'Tratativa'}
            </button>

            <button 
              onClick={() => onOpenCase(post.subject || post.caseId || post.title)}
              className="px-4 py-2.5 bg-slate-50 text-slate-600 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-slate-100 transition-all flex items-center gap-2 active:scale-95"
            >
              <Search className="w-3.5 h-3.5" />
              Cases
            </button>
      ...
      {/* MODAL DE AÇÕES RÁPIDAS (AGIR) — SUBSTITUI DROPDOWN PARA EVITAR CORTES */}
      {isAgirMenuOpen && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-[2px] z-[120] flex items-center justify-center p-4 animate-in fade-in duration-200">
           <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-sm overflow-hidden animate-in zoom-in-95 duration-200 border-[8px] border-white">
             <div className="bg-emerald-600 p-6 text-white flex items-center justify-between">
                <div>
                  ...
                  <p className="text-[10px] font-bold text-emerald-100 uppercase tracking-widest mt-1">Ações rápidas para este registro</p>
                </div>
                <button onClick={() => setIsAgirMenuOpen(false)} className="p-2 hover:bg-white/10 rounded-full transition-colors">
                  <X className="w-6 h-6" />
                </button>
             </div>
             ...
             <div className="p-2 space-y-1">
                <button 
                  onClick={() => {
                    setIsAgirMenuOpen(false);
                    setIsTreatmentOpen(true);
                  }}
                  className="w-full px-6 py-4 text-left text-[11px] font-black uppercase text-slate-700 hover:bg-slate-50 flex items-center justify-between group transition-all"
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p>{treatment ? 'Editar Tratativa' : 'Criar Tratativa'}</p>
                      <p className="text-[9px] text-slate-400 normal-case font-medium">Gerenciar workflow interno</p>
                    </div>
                  </div>
                ...
                <button 
                  onClick={() => {
                    setIsAgirMenuOpen(false);
                    if (post.caseId) onOpenCase(post.caseId);
                    else alert('Este post não possui case vinculado.');
                  }}
                  className="w-full px-6 py-4 text-left text-[11px] font-black uppercase text-slate-700 hover:bg-slate-50 flex items-center justify-between group transition-all"
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p>Abrir Case no SOVOS</p>
                      <p className="text-[9px] text-slate-400 normal-case font-medium">Ver detalhes originais do sistema</p>
                    </div>
                  </div>
                  <ChevronRight className="w-4 h-4 text-slate-300" />
                </button>
                ...
                <button 
                  onClick={() => {
                    setIsAgirMenuOpen(false);
                    onUpdate({ ...post, status: 'Tratado' });
                    if (treatment) onUpdateTreatment({ ...treatment, status: 'Concluída', encerrado_em: new Date().toISOString() });
                  }}
                  className="w-full px-6 py-4 text-left text-[11px] font-black uppercase text-emerald-600 hover:bg-emerald-50 flex items-center justify-between group transition-all"
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[9px] text-emerald-500/60 normal-case font-medium">Encerrar acompanhamento operacional</p>
                    </div>
                  </div>
                  <ChevronRight className="w-4 h-4 text-emerald-300" />
                </button>
                
                <div className="p-6 bg-slate-50 mt-2">
                   <p className="text-[10px] font-black text-slate-400 uppercase mb-4 tracking-widest text-center">Transferir Responsabilidade</p>
                   <div className="grid grid-cols-2 gap-2">
                     {users.slice(0, 4).map(u => (
                       <button 
                         ...
                         className="px-3 py-2 bg-white border border-slate-200 rounded-xl text-[10px] font-black text-slate-600 hover:border-emerald-500 hover:text-emerald-600 transition-all shadow-sm active:scale-95"
                       >
                         {u.name.split(' ')[0]}
                       </button>
                     ))}
      ...
      {/* Modal de Vínculo de Reincidência */}
      {isRelinkModalOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[110] flex items-center justify-center p-4">
          <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-md overflow-hidden border-[8px] border-white animate-in zoom-in-95 duration-200">
            <div className="bg-slate-900 text-white px-8 py-6 flex items-center justify-between">
              <div>
                <h3 className="text-lg font-black uppercase tracking-tight">Vincular Reincidência</h3>
                <p className="text-[9px] font-bold opacity-60 uppercase tracking-widest">Conectar este post a um histórico</p>
              </div>
              <button onClick={() => setIsRelinkModalOpen(false)} className="p-2 hover:bg-white/10 rounded-full transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="p-8 space-y-6">
              <div className="space-y-4">
                <p className="text-xs text-slate-500 font-medium">Selecione um post anterior ou um número de case para vincular como reincidência.</p>
                
                <div>
                  <label className="text-[10px] font-black text-slate-400 uppercase mb-1.5 block">Nº do Case (Original)</label>
                  <input 
                    type="text" 
                    placeholder="Ex: 01234567"
                    className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm font-bold outline-none focus:ring-2 focus:ring-slate-900"
                    onKeyDown={(e) => {
                      if (e.key === 'Enter') {
                        const val = (e.target as HTMLInputElement).value;
                        if (val) {
                          onUpdate({ ...post, reincidenceCaseId: val });
                          setIsRelinkModalOpen(false);
                          alert(`Vinculado ao case ${val}`);
                        }
                      }
                    }}
                  />
                ...
                <div className="border-t border-slate-100 pt-4">
                  <label className="text-[10px] font-black text-slate-400 uppercase mb-2 block">Posts Recentes (Possíveis Origens)</label>
                  <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
                    {posts.filter(p => p.id !== post.id && !p.isRemoved).slice(0, 5).map(p => (
                      <button 
                        key={p.id}
                        onClick={() => {
                          onUpdate({ ...post, reincidencePostId: p.id, reincidenceCaseId: p.caseId });
                          setIsRelinkModalOpen(false);
                          alert(`Vinculado ao post: ${p.title}`);
                        }}
                        className="w-full p-3 text-left border border-slate-100 rounded-xl hover:bg-slate-50 hover:border-slate-300 transition-all group"
                      >
                        <p className="text-[10px] font-black text-slate-700 uppercase group-hover:text-slate-900">{p.title}</p>
                        <p className="text-[8px] text-slate-400 font-bold uppercase">{format(new Date(p.createdAt), "dd/MM/yy")} {p.caseId ? `• Case ${p.caseId}` : ''}</p>
                      </button>
                    ))}
                  </div>
                </div>
              ...
              <div className="flex gap-3 pt-4">
                <button 
                  ...
                  className="flex-1 py-3 border-2 border-slate-100 text-slate-400 font-black uppercase text-[10px] rounded-xl hover:bg-slate-50 transition-all"
                >
                  Cancelar
                </button>
              </div>
